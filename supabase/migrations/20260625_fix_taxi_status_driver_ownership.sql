-- ════════════════════════════════════════════════════════════════════
-- FIX : update_taxi_trip_status — ownership chauffeur via taxi_drivers
-- ════════════════════════════════════════════════════════════════════
-- BUG : taxi_trips.driver_id = taxi_drivers.id (PAS auth.uid()). La RPC
-- comparait driver_id = auth.uid() → jamais vrai → annulation/complétion
-- refusées (« Non autorisé »). On vérifie désormais via taxi_drivers.user_id.
-- Le client reste vérifié par customer_id = auth.uid() (correct).
-- ════════════════════════════════════════════════════════════════════
BEGIN;
