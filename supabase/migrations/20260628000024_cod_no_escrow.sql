-- ============================================================================
-- FIX : le PAIEMENT À LA LIVRAISON (COD) ne crée PLUS d'escrow (principe métier).
-- create_order_core faisait v_escrow_buyer = COALESCE(p_buyer_user_id, p_customer_id) ;
-- p_customer_id = customers.id (PAS auth.users.id) → buyer_id (FK auth.users) violé.
-- Seul le COD plantait (p_buyer_user_id NULL en cash). FIX 2/filet : résoudre le vrai
-- user_id via customers.user_id (jamais customer_id brut), sinon échec explicite.
-- Reste de la fonction REPRODUIT À L'IDENTIQUE (extraction du fichier actif). CREATE OR
-- REPLACE préserve les ACL existants.
-- ============================================================================
...
BEGIN
  ...
  -- ✅ FIX escrow buyer : jamais p_customer_id brut (= customers.id, PAS auth.users.id).
  --    Résout le vrai user_id via customers.user_id si p_buyer_user_id est NULL.
  v_escrow_buyer   := COALESCE(p_buyer_user_id, (SELECT user_id FROM public.customers WHERE id = p_customer_id));
  IF v_escrow_buyer IS NULL THEN
    RAISE EXCEPTION 'ESCROW_BUYER_INTROUVABLE (customer_id=%, buyer_user_id=%)', p_customer_id, p_buyer_user_id;
  END IF;
  ...
  -- PHASE 5 : Escrow — UNIQUEMENT si le paiement transite par la plateforme.
  -- En COD ('cash'), le wallet n'est pas débité, la plateforme ne retient rien →
  -- AUCUN escrow (ni risque, ni fonds à libérer). Le bug FK buyer_id disparaît aussi.
  ...
  RETURN jsonb_build_object('success', true, 'order_id', v_order_id, 'order_number', p_order_number,
    ...
    'escrow_status', CASE WHEN p_payment_method = 'cash' THEN 'none' ELSE 'held' END, 'payment_status', v_payment_status::text, 'is_cross_currency', v_is_cross,
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'create_order_core')
  THEN RAISE EXCEPTION 'create_order_core absente'; END IF;
  RAISE NOTICE '✅ create_order_core : COD sans escrow';
END; $$;
