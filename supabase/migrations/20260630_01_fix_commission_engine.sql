-- ════════════════════════════════════════════════════════════════════════════
-- ÉTAPE 1 — MOTEUR COMMISSION : fin du MINT, la part agent est PRÉLEVÉE sur le PDG.
-- ════════════════════════════════════════════════════════════════════════════
-- BUG corrigé : credit_agent_wallet_gnf crédite l'agent SANS débiter personne →
-- le PDG encaisse 100% des frais ET les agents reçoivent 20% mintés = fuite de 20%.
--
-- FIX : credit_agent_commission débite désormais le wallet PDG du TOTAL réellement
-- crédité aux agents (transfert, conservation). UN SEUL débit par appel, dans la
-- MÊME transaction que les crédits.
--   • PDG résolu via pdg_management.user_id + wallet GNF (pas de 'pdg_wallet_id' —
--     inexistant ; wallets.id est un entier, clé métier = user_id+currency).
--   • Garde anti-solde-négatif : si le PDG ne peut pas payer → commission NON versée
--     (fail-closed, jamais de mint), le paiement appelant réussit quand même.
--   • Idempotence précoce par (transaction_id, related_user_id) + ON CONFLICT
--     (agent_id, transaction_id) conservé → pas de double crédit ni double débit.
--   • credit_agent_wallet_gnf INCHANGÉ (ne fait que créditer l'agent).
-- Reste de la fonction reproduit À L'IDENTIQUE.
-- ════════════════════════════════════════════════════════════════════════════
BEGIN;
...
DO $$ BEGIN
  RAISE NOTICE '✅ credit_agent_commission : part agent prélevée sur le PDG (fin du mint)';
END $$;
