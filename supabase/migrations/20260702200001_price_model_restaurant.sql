-- ============================================================================
-- MODÈLE DE PRIX UNIFORME — RESTAURANT (le restaurateur reçoit ses PLATS complets)
-- ----------------------------------------------------------------------------
-- Avant : restaurant crédité de `p_amount − commission` (v_net, amputé). Après :
-- le CLIENT paie `plats + commission (+ frais livraison)`, le RESTAURANT reçoit
-- les plats COMPLETS (p_amount), le PDG reçoit la commission.
-- La logique de LIVRAISON (v_fee / v_fee_payer / séquestre / reversement livreur
-- pay_restaurant_delivery + trigger) est INCHANGÉE — seule la commission change de
-- porteur (client au lieu du restaurant). Invariant : débit == plats + commission +
-- frais client == crédit resto (plats) + PDG (commission) + PDG (frais séquestre).
-- ============================================================================
...
BEGIN
  ...
  -- Commission sur les PLATS uniquement (pas sur les frais de livraison), payée par le CLIENT en plus.
  ...
  -- (2) CRÉDIT RESTAURANT = plats COMPLETS (plus d'amputation de la commission).
  v_owner_res := public.credit_user_wallet_safe(v_owner, p_amount, v_cur);
  v_got := COALESCE((v_owner_res->>'credited')::numeric, 0) + COALESCE((v_owner_res->>'quarantined')::numeric, 0);
  IF p_amount > 0 AND v_got <= 0 THEN RAISE EXCEPTION 'CREDIT_RESTAURANT_ECHOUE'; END IF;
...
SELECT 'Modèle prix restaurant : restaurateur reçoit les plats complets, commission payée par le client (livraison inchangée).' AS status;
