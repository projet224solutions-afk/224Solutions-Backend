-- ============================================================================
-- 🛡️ MARKETPLACE — Boutique PHYSIQUE uniquement : produits non commandables en ligne
-- ----------------------------------------------------------------------------
-- Décision PDG : une boutique `business_type='physical'` garde son PROFIL visible sur
-- le marketplace, mais ses PRODUITS ne sont ni affichés ni COMMANDABLES en ligne tant
-- qu'elle n'active pas 'hybrid' (physique + en ligne) ou 'online'. Verrou serveur :
-- create_order_core refuse toute commande dont le vendeur est physical-only.
--
-- Recréée DEPUIS SA DÉFINITION LIVE (leçon du garde FX) — SEUL ajout : le garde en tête.
-- N'affecte PAS le POS du vendeur (ventes en boutique via create_pos_sale_complete, RPC
-- distincte). Le garde lève VENDOR_PHYSICAL_ONLY, capté par le WHEN OTHERS final →
-- retour {success:false, error:'VENDOR_PHYSICAL_ONLY'} mappé en FR côté backend.
-- ============================================================================

CREATE OR REPLACE FUNCTION public.create_order_core(p_order_number text, p_customer_id uuid, p_vendor_id uuid, p_vendor_user_id uuid, p_payment_method text, p_payment_intent_id text DEFAULT NULL::text, p_shipping_address jsonb DEFAULT '{}'::jsonb, p_currency text DEFAULT 'GNF'::text, p_items jsonb DEFAULT '[]'::jsonb, p_auto_release_days integer DEFAULT 7, p_buyer_user_id uuid DEFAULT NULL::uuid, p_wallet_debit_amount numeric DEFAULT 0, p_buyer_wallet_currency text DEFAULT NULL::text, p_exchange_rate_used numeric DEFAULT NULL::numeric, p_buyer_fee_amount numeric DEFAULT 0, p_seller_commission_amount numeric DEFAULT NULL::numeric)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
DECLARE
  ...
  v_max_age_hours     int;
BEGIN
  -- 🛡️ GARDE BOUTIQUE PHYSIQUE : profil visible mais vente en ligne désactivée.
  -- N'affecte PAS le POS (create_pos_sale_complete). Le vendeur active 'hybrid'/'online' pour vendre.
  IF (SELECT business_type FROM public.vendors WHERE id = p_vendor_id) = 'physical' THEN
    RAISE EXCEPTION 'VENDOR_PHYSICAL_ONLY';
  END IF;

  v_wallet_cur     := COALESCE(p_buyer_wallet_currency, p_currency);
  v_is_cross       := (p_currency IS DISTINCT FROM v_wallet_cur);
  -- Filet : jamais p_customer_id brut (= customers.id) dans buyer_id.
  v_escrow_buyer   := COALESCE(p_buyer_user_id, (SELECT user_id FROM public.customers WHERE id = p_customer_id));
  ...
  -- PHASE 5 : Escrow — UNIQUEMENT hors COD (en cash, la plateforme ne retient rien).
  ...
  RETURN jsonb_build_object('success', true, 'order_id', v_order_id, 'order_number', p_order_number,
    ...
    'escrow_status', CASE WHEN p_payment_method = 'cash' THEN 'none' ELSE 'held' END,
    'payment_status', v_payment_status::text, 'is_cross_currency', v_is_cross,
...
-- ── Auto-test : garde présent + une seule surcharge ─────────────────────────
DO $$
DECLARE v_cnt int; v_src text;
BEGIN
  SELECT count(*) INTO v_cnt FROM pg_proc WHERE proname = 'create_order_core';
  IF v_cnt <> 1 THEN RAISE EXCEPTION 'create_order_core : % surcharges (attendu 1)', v_cnt; END IF;
  SELECT prosrc INTO v_src FROM pg_proc WHERE proname = 'create_order_core';
  IF v_src NOT LIKE '%VENDOR_PHYSICAL_ONLY%' THEN
    RAISE EXCEPTION 'create_order_core : garde VENDOR_PHYSICAL_ONLY absent';
  END IF;
  RAISE NOTICE 'OK : create_order_core unique + garde physical-only présent.';
END $$;

SELECT 'Garde physical-only ajouté à create_order_core (profil visible, vente en ligne verrouillée).' AS status;
