-- ============================================================================
-- IMAGES/CATÉGORIES des produits reçus : complétion à l'appariement + BACKFILL.
-- Diagnostic (piste 3) : la copie ne tournait qu'à la CRÉATION → les produits
-- appariés/anciens restaient sans image ni catégorie. categories = GLOBALE (l'id
-- copié est valide) ; bucket product-images PUBLIC (URLs lisibles).
-- ============================================================================
...
-- BACKFILL (non destructif : remplit UNIQUEMENT les trous depuis le produit
-- fournisseur, via le lien stock_purchase_items.supplier_product_id → product_id).
UPDATE public.products bp SET
  images = CASE WHEN (bp.images IS NULL OR array_length(bp.images,1) IS NULL)
                  AND sp.images IS NOT NULL AND array_length(sp.images,1) IS NOT NULL
                THEN sp.images ELSE bp.images END,
  category_id = COALESCE(bp.category_id, sp.category_id),
  description = COALESCE(NULLIF(bp.description,''), sp.description),
  barcode = COALESCE(NULLIF(bp.barcode,''), sp.barcode),
  weight = COALESCE(bp.weight, sp.weight),
  dimensions = COALESCE(bp.dimensions, sp.dimensions),
  updated_at = now()
...
WHERE bp.id = spi.product_id
  ...
  AND (((bp.images IS NULL OR array_length(bp.images,1) IS NULL) AND sp.images IS NOT NULL AND array_length(sp.images,1) IS NOT NULL)
       OR (bp.category_id IS NULL AND sp.category_id IS NOT NULL)
       OR (bp.description IS NULL AND sp.description IS NOT NULL)
       OR (bp.barcode IS NULL AND sp.barcode IS NOT NULL));
