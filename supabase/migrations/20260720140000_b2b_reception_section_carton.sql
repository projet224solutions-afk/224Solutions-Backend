-- ============================================================================
-- RÉCEPTION B2B : SECTION (sous-catégorie) + CONDITIONNEMENT CARTON copiés/complétés.
-- Même mécanique que les images/catégories : à la réception, le produit ACHETEUR
-- récupère du produit FOURNISSEUR sa 'section' (regroupe la barre du POS) et sa
-- config carton (sell_by_carton, units_per_carton) — COALESCE, jamais d'écrasement.
-- Le PRIX CARTON n'est PAS copié : il est calculé sur le prix de vente UNITAIRE de
-- l'acheteur (× units_per_carton, arrondi commercial) dans apply_reception_pricing.
-- + BACKFILL des produits déjà reçus (section, carton, prix carton recalculé).
-- 'section' = colonne texte de products (pas de table dédiée) → simple copie de libellé.
-- ============================================================================
...
BEGIN
  ...
  FOR v_line IN SELECT * FROM jsonb_array_elements(p_lines) LOOP
    ...
    ELSE
      ...
      IF v_buyer_product IS NULL THEN
        INSERT INTO public.products (vendor_id, name, price, stock_quantity, is_active, category_id,
          description, images, sku, barcode, weight, dimensions, tags, section,
        ...
        VALUES (
          ...
          v_sup.barcode,
          v_sup.weight, v_sup.dimensions, v_sup.tags, v_sup.section,
    ...
    -- 🖼️ COMPLÉTION médias/métadonnées (jamais d'écrasement) : un produit acheteur
    -- MATCHÉ/existant sans image/catégorie/... récupère ces champs du produit fournisseur.
    IF v_sup.id IS NOT NULL AND v_buyer_product IS NOT NULL THEN
      UPDATE public.products bp SET
        images = CASE WHEN (bp.images IS NULL OR array_length(bp.images,1) IS NULL)
                        AND v_sup.images IS NOT NULL AND array_length(v_sup.images,1) IS NOT NULL
                      THEN v_sup.images ELSE bp.images END,
        category_id = COALESCE(bp.category_id, v_sup.category_id),
        description = COALESCE(NULLIF(bp.description,''), v_sup.description),
        barcode = COALESCE(NULLIF(bp.barcode,''), v_sup.barcode),
        weight = COALESCE(bp.weight, v_sup.weight),
        dimensions = COALESCE(bp.dimensions, v_sup.dimensions),
        section = COALESCE(NULLIF(bp.section,''), v_sup.section),
        sell_by_carton = CASE WHEN COALESCE(bp.units_per_carton,1) <= 1 AND COALESCE(v_sup.units_per_carton,1) > 1
                              THEN COALESCE(v_sup.sell_by_carton, bp.sell_by_carton) ELSE bp.sell_by_carton END,
        units_per_carton = CASE WHEN COALESCE(bp.units_per_carton,1) <= 1 AND COALESCE(v_sup.units_per_carton,1) > 1
                                THEN v_sup.units_per_carton ELSE bp.units_per_carton END,
        updated_at = now()
      WHERE bp.id = v_buyer_product
        AND ((bp.images IS NULL OR array_length(bp.images,1) IS NULL)
             OR bp.category_id IS NULL OR bp.description IS NULL
             OR bp.barcode IS NULL OR bp.weight IS NULL OR bp.dimensions IS NULL
             OR bp.section IS NULL OR bp.section = ''
             OR (COALESCE(bp.units_per_carton,1) <= 1 AND COALESCE(v_sup.units_per_carton,1) > 1));
    END IF;
    ...
    v_report := v_report || jsonb_build_object(
      ...
      'buyer_product_id', v_buyer_product, 'created', v_created, 'received_now', v_recv,
...
CREATE OR REPLACE FUNCTION public.apply_reception_pricing(p_vendor_id uuid, p_lines jsonb, p_reason text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
DECLARE
  ...
  v_sbc boolean; v_upc int; v_step int; v_carton numeric;
BEGIN
  ...
  SELECT rounding_step INTO v_step FROM public.vendor_pricing_settings WHERE vendor_id = p_vendor_id;
  ...
  FOR v_line IN SELECT * FROM jsonb_array_elements(p_lines) LOOP
    ...
    SELECT category_id, price, sell_by_carton, units_per_carton INTO v_cat, v_current, v_sbc, v_upc FROM public.products WHERE id = v_pid AND vendor_id = p_vendor_id;
    ...
    -- 📦 Prix de vente au CARTON de l'ACHETEUR = son prix unitaire × unités/carton,
    -- arrondi au palier commercial (JAMAIS copié du fournisseur).
    IF COALESCE(v_sbc, false) AND COALESCE(v_upc, 1) > 1 AND v_effective IS NOT NULL AND v_effective > 0 THEN
      v_carton := round((v_effective * v_upc) / v_step) * v_step;
      UPDATE public.products SET price_carton = v_carton, updated_at = now()
      WHERE id = v_pid AND COALESCE(price_carton, 0) IS DISTINCT FROM v_carton;
    END IF;
  END LOOP;
...
-- ── BACKFILL 1 : section + carton structurel (trous uniquement, via le lien
--    stock_purchase_items.supplier_product_id → product_id). Jamais d'écrasement. ──
UPDATE public.products bp SET
  section = COALESCE(NULLIF(bp.section,''), sp.section),
  sell_by_carton = CASE WHEN COALESCE(bp.units_per_carton,1) <= 1 AND COALESCE(sp.units_per_carton,1) > 1
                        THEN COALESCE(sp.sell_by_carton, bp.sell_by_carton) ELSE bp.sell_by_carton END,
  units_per_carton = CASE WHEN COALESCE(bp.units_per_carton,1) <= 1 AND COALESCE(sp.units_per_carton,1) > 1
                          THEN sp.units_per_carton ELSE bp.units_per_carton END,
  updated_at = now()
FROM public.stock_purchase_items spi
JOIN public.products sp ON sp.id = spi.supplier_product_id
WHERE bp.id = spi.product_id
  AND spi.supplier_product_id IS NOT NULL
  AND ((bp.section IS NULL OR bp.section = '')
       OR (COALESCE(bp.units_per_carton,1) <= 1 AND COALESCE(sp.units_per_carton,1) > 1));

-- ── BACKFILL 2 : prix carton = prix unitaire ACTUEL de l'acheteur × units_per_carton,
--    arrondi au palier du vendeur (défaut 1000). Recalcul explicite (pas une copie). ──
UPDATE public.products bp SET
  price_carton = round((bp.price * bp.units_per_carton)
                 / COALESCE((SELECT rounding_step FROM public.vendor_pricing_settings WHERE vendor_id = bp.vendor_id), 1000)::numeric)
                 * COALESCE((SELECT rounding_step FROM public.vendor_pricing_settings WHERE vendor_id = bp.vendor_id), 1000),
  updated_at = now()
WHERE bp.id IN (SELECT DISTINCT product_id FROM public.stock_purchase_items
                WHERE supplier_product_id IS NOT NULL AND product_id IS NOT NULL)
  AND bp.sell_by_carton = true AND COALESCE(bp.units_per_carton,1) > 1
  AND COALESCE(bp.price,0) > 0;
