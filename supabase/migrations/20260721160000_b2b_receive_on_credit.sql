-- ============================================================================
-- B2B — RÉCEPTION « À CRÉDIT » avec PRÉ-AUTORISATION du fournisseur (P2.3).
--
-- Constat : receive_b2b_purchase est figée sur le mode de la commande. Pour une commande
-- wallet/on_reception, un acheteur sans fonds ne pouvait ni payer ni recevoir → bloqué.
-- Décision PDG : autoriser le règlement À CRÉDIT à la réception, MAIS uniquement si le
-- FOURNISSEUR l'a pré-autorisé (sinon une dette lui serait imposée sans son accord).
--
-- 1) `stock_purchases.credit_on_reception_allowed` (bool) — le fournisseur l'active (défaut false).
-- 2) receive_b2b_purchase(+ p_settle_credit boolean) : dans la branche wallet/on_reception,
--    si p_settle_credit ET credit_on_reception_allowed → crée une dette (supplier_debts) +
--    dépense 'credit' pending, AU LIEU de débiter le wallet. Si demandé sans autorisation →
--    RAISE 'B2B_RECV:CREDIT_NOT_ALLOWED'. Sinon, débit wallet inchangé.
-- Signature élargie → on DROP l'ancienne (8 args) pour éviter l'ambiguïté PostgREST.
--
-- Idempotente (ADD COLUMN IF NOT EXISTS + DROP IF EXISTS + CREATE OR REPLACE).
-- ============================================================================

ALTER TABLE public.stock_purchases
  ADD COLUMN IF NOT EXISTS credit_on_reception_allowed boolean NOT NULL DEFAULT false;

DROP FUNCTION IF EXISTS public.receive_b2b_purchase(uuid, uuid, jsonb, boolean, text, numeric, text, numeric);

CREATE OR REPLACE FUNCTION public.receive_b2b_purchase(p_purchase_id uuid, p_buyer_vendor_id uuid, p_lines jsonb, p_close boolean DEFAULT false, p_note text DEFAULT NULL::text, p_wallet_debit_amount numeric DEFAULT 0, p_buyer_wallet_currency text DEFAULT NULL::text, p_buyer_fee_amount numeric DEFAULT 0, p_settle_credit boolean DEFAULT false)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  ELSIF v_purchase.payment_mode = 'wallet' AND v_purchase.payment_timing = 'on_reception' THEN
    -- 💳 P2.3 : règlement À CRÉDIT à la réception si l'acheteur le demande ET que le
    -- FOURNISSEUR l'a pré-autorisé (credit_on_reception_allowed). Sinon débit wallet.
    IF p_settle_credit AND COALESCE(v_purchase.credit_on_reception_allowed, false) THEN
      ...
      INSERT INTO public.vendor_expenses (vendor_id, description, amount, expense_date,
        payment_method, status, is_locked, purchase_reference)
      VALUES (v_buyer_user, v_desc || ' (à crédit, réception)', v_received_value, CURRENT_DATE,
        'credit', 'pending', true, v_order.order_number)
      RETURNING id INTO v_expense_id;
    ELSIF p_settle_credit THEN
      RAISE EXCEPTION 'B2B_RECV:CREDIT_NOT_ALLOWED:le fournisseur n''autorise pas le crédit à la réception';
    ELSE
      ...
      IF upper(v_wallet_cur) = upper(COALESCE(v_purchase.currency, 'GNF'))
        ...
        RAISE EXCEPTION 'B2B_RECV:DEBIT_MISMATCH:% attendu, % donné', v_received_value, COALESCE(p_wallet_debit_amount, 0);
      END IF;
      IF COALESCE(p_wallet_debit_amount, 0) <= 0 THEN
        RAISE EXCEPTION 'B2B_RECV:INVALID_WALLET_AMOUNT:montant de débit requis (>0)';
      END IF;
      ...
      IF NOT FOUND THEN
        RAISE EXCEPTION 'B2B_RECV:WALLET_NOT_FOUND:%', v_wallet_cur;
      END IF;
      IF v_bal < v_total_debit THEN
        RAISE EXCEPTION 'B2B_RECV:INSUFFICIENT_FUNDS:solde % < requis %', v_bal, v_total_debit;
      END IF;
...
$function$
;
REVOKE ALL ON FUNCTION public.receive_b2b_purchase(uuid,uuid,jsonb,boolean,text,numeric,text,numeric,boolean) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.receive_b2b_purchase(uuid,uuid,jsonb,boolean,text,numeric,text,numeric,boolean) TO service_role;

SELECT 'receive_b2b_purchase : règlement à crédit à la réception si pré-autorisé par le fournisseur (p_settle_credit + credit_on_reception_allowed).' AS status;
