-- Expose la map code->compte (sc.m, déjà calculée) + debug dans get_proximity_stats
-- Permet à la page Proximité de compter N'IMPORTE quel service_type par code (page dynamique).
-- Corps IDENTIQUE au déployé, seule addition : 'by_code', sc.m au niveau racine.
...
-- â”€â”€ Produits : ILIKE nom catÃ©gorie, COUNT DISTINCT, SANS GPS â”€â”€
-- âš ï¸� CORRECTIF : seuls les produits de vendeurs online/hybrid (mÃªmes que la liste client).
prod AS (
...
prod_counts AS (
  SELECT
    ...
    count(DISTINCT id) FILTER (WHERE cname LIKE '%maison%' OR cname LIKE '%dÃ©co%' OR cname LIKE '%home%')::int AS maison
  FROM prod
)
